...
##################
using_tello = 1 # poner a 1 si se tiene el dron
tello_cam = 1  # poner a 1 si se tiene el dron y sólo se quiere usar la cámara
detector = 0 # 0: color, 1: mano
num_total_puertas = 100 # número total de puertas en el circuito  
##################
...
while True:
    ...
    diffant = diff
    dir, diff = direction_control(cx,cy,object_detected, frameWidth, frameHeight, deadZone, imgContour)
    ...
    if using_tello:
        ...
        cnt_sec, num_puertas, atrav_puerta = movement_control(drone, dir, cnt_sec, num_puertas, atrav_puerta, diff)
    else:
        cnt_sec, num_puertas, atrav_puerta = movement_control_sim(dir,cnt_sec, num_puertas, atrav_puerta, diff)
